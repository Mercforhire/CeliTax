...
- (void)initTranslucentView {
	if (SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"7.0")) {
		...
		CGRect shadow = self.translucentView.bounds;
		shadow.size.width = 2;

		UIBezierPath *shadowPath = [UIBezierPath bezierPathWithRect:shadow];
		self.translucentView.layer.masksToBounds = NO;
		self.translucentView.layer.shadowColor = [UIColor blackColor].CGColor;
		self.translucentView.layer.shadowOffset = CGSizeMake(0.0f, 5.0f);
		self.translucentView.layer.shadowOpacity = 0.8f;
		self.translucentView.layer.shadowPath = shadowPath.CGPath;
